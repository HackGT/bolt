...
export class RequestedItemsByUser extends Component<RequestedItemsByUserProps> {
    ...
    render() {
        const leftButton = this.props.backToPrep ? (
            <Button onClick={this.onClickPrep} color='orange' size='mini' icon labelPosition="left">Prep<Icon name="arrow alternate circle left outline"/></Button>
        ) :
        (
            <div></div>
        );
        ...
        return (
            <Card>
                <Card.Header style={{padding: "1rem"}}>
                    ...
                    <Button.Group fluid>
                        {leftButton}
                        <Popup trigger={<Button onClick={this.onClickCross} color='red' size='mini'><Icon name="close"/></Button>} content="Remove User block"/>
                        {rightButton}
                    </Button.Group>
                </Card.Header>
                        ...
                        <Card.Content>
                            ...
                            <Card.Content extra>
                              <div className='ui two buttons' style={{paddingTop: "1rem"}}>
                                <Popup trigger={<Button basic color='grey'><Icon name="edit"/></Button>} content="Edit Quantity Requested" />
                                <Popup trigger={<Button basic color='red'><Icon name="close"/></Button>} content="Remove item" />
                              </div>
                            </Card.Content>
                        </Card.Content>
                    )
