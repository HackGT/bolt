...
const CardList: React.FunctionComponent<CardListProps> = props => {
  ...
  const cards = props.cards
    .filter((elem: any) => props.filter(elem, searchQuery))
    .map((elem: any) => props.render(elem));
  const noResults = (
    <Segment placeholder>
      <Container textAlign="center">
        <Header>No search results found</Header>
      </Container>
    </Segment>
  );
  ...
  return (
    <Grid.Column>
      ...
      <Segment attached="top">
        {props.length} request{props.length === 1 ? "" : "s"}
      </Segment>
      <Container className="hw-list">{content}</Container>
    </Grid.Column>
  );
};
